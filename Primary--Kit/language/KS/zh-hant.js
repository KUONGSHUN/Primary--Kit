//-----------------NOVA---------------------------------
Blockly.LKL_DIGITAL_OUT = '數字輸出';
Blockly.LKL_NOVA_HIGH = '高';
Blockly.LKL_NOVA_LOW = '低';
Blockly.LKL_DIGITAL_IN = '數字輸入';
Blockly.LKL_NOVA_ANALOG_WRITE = '模擬輸出';
Blockly.LKL_NOVA_PWM_SET = '赋值为';
Blockly.LKL_Baron_G = '绿燈赋值为';
Blockly.LKL_Baron_R = '红燈赋值为';
Blockly.LKL_Baron_B = '蓝燈赋值为';
Blockly.LKL_Baron_Y = '黄燈赋值为';
Blockly.LKL_NOVA_ANALOG_READ = '模擬輸入'
Blockly.LKL_Baron_IR_Data='接收值 ';
Blockly.LKL_Baron_IR_RECEIVE='紅外接收';
Blockly.LKL_Baron_IR_Key = '紅外遙控器鍵值';

Blockly.LKL_NOVA_PORT = '端口';
Blockly.LKL_NOVA_PORT1 = '端口  E    ';
Blockly.LKL_NOVA_STAT = '設為';
Blockly.LKL_NOVA_GET_STAT = '獲取';

// LED
Blockly.LKL_NOVA_LED = 'LED燈';
Blockly.LKL_Baron_RGB = 'RGB燈';
Blockly.LKL_Baron_LED1 = '交通燈'; 
Blockly.LKL_Baron_LED = '紅外鐳射頭';
Blockly.LKL_NOVA_LED_ON = '開';
Blockly.LKL_NOVA_LED_OFF = '關';
Blockly.LKL_NOVA_LED_PWM = '設置亮度為(0~100)%';

// GLLED
Blockly.LKL_NOVA_GLLED = '高亮LED';
Blockly.LKL_NOVA_GLLED_PWM = '設置亮度為(0~100)%';
Blockly.LKL_NOVA_GLLED_ON = '開';
Blockly.LKL_NOVA_GLLED_OFF = '關'; 



//PMR3
Blockly.MIXLY_YFPMR3_MOTOR_SPEED='轉速';
Blockly.MIXLY_YFPMR3='板載電機驅動';
Blockly.MIXLY_YFPMR3_MOTOR='電機';
Blockly.MIXLY_YFPMR3_MOTOR_SPEED_RANGE='(0~255)';
Blockly.MIXLY_YFPMR3_MOTOR_DIRECTION_F='正轉';
Blockly.MIXLY_YFPMR3_MOTOR_DIRECTION_D='反轉';

// Button
Blockly.LKL_NOVA_BUTTON_PRESSED = '按下';
Blockly.LKL_NOVA_BUTTON_RELEASED = '鬆開';
Blockly.LKL_BARON_TOUCH = '觸摸';
Blockly.LKL_NOVA_BTN = '按鈕';
Blockly.LKL_NOVA_ABCD = '#';

Blockly.LKL_NOVA_TILTSWITCH = '傾斜開關';
Blockly.LKL_NOVA_TiltSwitch_UP='傾斜';
Blockly.LKL_NOVA_TiltSwitch_DOWN='水平';
Blockly.LKL_NOVA_SOUND = '聲音'; 
Blockly.LKL_NOVA_LIMISWICTH = '限位開關'; 
Blockly.LKL_NOVA_LIGHT = '光敏'; 

Blockly.LKL_Baron_BUZZER = '有源蜂鳴器'; 
Blockly.LKL_Baron_BUZZER1 = '無源蜂鳴器';
Blockly.LKL_NOVA_FREQUENCY = '頻率(Hz)'; 
Blockly.LKL_NOVA_LKL_DURATION = '持續時間(ms)'; 

Blockly.LKL_NOVA_MOTOR = '單路直流電機'; 
Blockly.LKL_NOVA_MOTOR_SPEED='轉速(0~100)%';


Blockly.LKL_Baron_SR501='HC-SR501';

Blockly.LKL_NOVA_ULTRASONIC = '超聲波測距(4~200cm)'; 

Blockly.LKL_NOVA_DS18B20 = '溫度DS18B20';
Blockly.LKL_NOVA_DS18B20_getTemp = '獲取溫度'; 

Blockly.LKL_NOVA_DHT11 = '溫濕度';
Blockly.LKL_NOVA_TYPE = '類型';
Blockly.LKL_NOVA_DHT11_H = '獲取濕度';
Blockly.LKL_NOVA_DHT11_T = '獲取溫度';

Blockly.LKL_NOVA_SERVO = '小舵機';
Blockly.LKL_NOVA_SERVO_BIG = '大舵機驅動';
Blockly.LKL_NOVA_SERVO_BRANCH = '分支分支';
Blockly.LKL_NOVA_DEGREE_0_180 = '角度(0~180)';
Blockly.LKL_NOVA_DELAY = '延時';
Blockly.LKL_NOVA_DELAY_MS = '毫秒';

Blockly.LKL_NOVA_Joystick = '搖桿';
Blockly.LKL_NOVA_readX = 'X值';
Blockly.LKL_NOVA_readY = 'Y值';
Blockly.LKL_Baron_JOYSTICK_D='搖桿-按鈕';
Blockly.LKL_Baron_JOYSTICK_A='搖桿';

Blockly.LKL_NOVA_LINEFINDER = '灰度';
Blockly.LKL_NOVA_L = '左邊狀態';
Blockly.LKL_NOVA_R = '右邊狀態';
Blockly.LKL_NOVA_READ = '獲取灰度狀態';

Blockly.LKL_NOVA_4DIGITDISPLAY = '數碼管';
Blockly.LKL_NOVA_NUMBER = '數字';
Blockly.LKL_NOVA_TIME = '時間';
Blockly.LKL_NOVA_DISPLAY_NUMBER = '數值';
Blockly.LKL_NOVA_DISPLAY_CLEAR= '清空第幾位(1/2/3/4)';

Blockly.LKL_NOVA_RTC = '時鐘';
Blockly.LKL_NOVA_RTC_SEC = '秒';
Blockly.LKL_NOVA_RTC_MIN = '分';
Blockly.LKL_NOVA_RTC_HOUR = '時';
Blockly.LKL_NOVA_RTC_WEEK = '星期';
Blockly.LKL_NOVA_RTC_DAY = '日';
Blockly.LKL_NOVA_RTC_MON = '月';
Blockly.LKL_NOVA_RTC_YEAR = '年';

Blockly.LKL_NOVA_RTC_SET_SEC = '設置秒(0~59)';
Blockly.LKL_NOVA_RTC_SET_MIN = '設置分(0~59)';
Blockly.LKL_NOVA_RTC_SET_HOUR = '設置時(0~23)';
Blockly.LKL_NOVA_RTC_SET_WEEK = '設置星期(1~7)';
Blockly.LKL_NOVA_RTC_SET_DAY = '設置日';
Blockly.LKL_NOVA_RTC_SET_MON = '設置月';
Blockly.LKL_NOVA_RTC_SET_YEAR = '設置年(>2000)';

Blockly.LKL_NOVA_IR_REC = '紅外鍵值';
Blockly.LKL_NOVA_IR_REC_AVAILABLE = '紅外接收有信號';

Blockly.LKL_NOVA_BLUETOOTH = '藍牙';
Blockly.LKL_NOVA_BLUETOOTH_AVAILABLE = '藍牙有數據可讀？';
Blockly.LKL_NOVA_BLUETOOTH_READ_STR = '藍牙讀取數據（返回字符串）';
Blockly.LKL_NOVA_BLUETOOTH_READ_NUM = '藍牙讀取數據（返回數字）';

Blockly.LKL_NOVA_VIBRATION='震動開關';
Blockly.LKL_NOVA_Vibration_Vibrating='有震動';
Blockly.LKL_NOVA_Vibration_NoVibrating='無震動';

// RGB
Blockly.LKL_NOVA_RGB = 'RGB燈';
Blockly.LKL_NOVA_RGB_NUM = '燈號';
Blockly.LKL_NOVA_RGB_R= 'R值';
Blockly.LKL_NOVA_RGB_G= 'G值';
Blockly.LKL_NOVA_RGB_B= 'B值';

Blockly.LKL_Nova_Matrix_POS='點陣座標';
Blockly.LKL_NOVA_Matrix='點陣';
Blockly.LKL_NOVA_DotMatrixName='幀數';
Blockly.LKL_NOVA_Matrix_x='x座標(1~8)';
Blockly.LKL_NOVA_Matrix_y='y座標(1~8)';
Blockly.LKL_NOVA_Matrix_Str='輸出字符串';
Blockly.LKL_Nova_DotMatrix='點陣屏幕:'
Blockly.LKL_Nova_MatrixRow0='第一行:';
Blockly.LKL_Nova_MatrixRow1='第二行:';
Blockly.LKL_Nova_MatrixRow2='第三行:';
Blockly.LKL_Nova_MatrixRow3='第四行:';
Blockly.LKL_Nova_MatrixRow4='第五行:';
Blockly.LKL_Nova_MatrixRow5='第六行:';
Blockly.LKL_Nova_MatrixRow6='第七行:';
Blockly.LKL_Nova_MatrixRow7='第八行:';

Blockly.LKL_NOVA_STEPPER='步進電機';
Blockly.LKL_NOVA_POSITION='設置目標位子';
Blockly.LKL_NOVA_STEPPER_RUN='步進電機執行動作';

Blockly.LKL_NOVA_MP3='MP3';
Blockly.LKL_NOVA_VOL='音量(0~100)%';
Blockly.LKL_NOVA_PLAY='播放曲目';
Blockly.LKL_NOVA_MP3_ONPLAY='播放';
Blockly.LKL_NOVA_MP3_STATE='播放模式';
Blockly.LKL_NOVA_MP3_LOOPPLAY='循環播放';
Blockly.LKL_NOVA_MP3_RANDPLAY='隨機播放';
Blockly.LKL_NOVA_MP3_PAUSE='暫停';
Blockly.LKL_NOVA_MP3_STOP='停止';

Blockly.LKL_NOVA_GYRO='陀螺儀';
Blockly.LKL_NOVA_GYRO_UPDATE='陀螺儀更新數據';
Blockly.LKL_NOVA_GYRO_STATE='坐標';
Blockly.LKL_NOVA_GYRO_X='X坐標';
Blockly.LKL_NOVA_GYRO_Y='Y坐標';

Blockly.LKL_NOVA_MQ='氣體';

Blockly.LKL_NOVA_DUAL_MOTOR='雙路電機';
Blockly.LKL_NOVA_DUAL_MOTORA='電機1';
Blockly.LKL_NOVA_DUAL_MOTORB='電機2';
Blockly.LKL_NOVA_DUAL_MOTOR_SPEEDA='轉速1(-100~100)%';
Blockly.LKL_NOVA_DUAL_MOTOR_SPEEDB='轉速2(-100~100)%';

Blockly.LKL_NOVA_FLAME='火焰';

Blockly.LKL_NOVA_HALL='霍爾開關';
Blockly.LKL_NOVA_Hall_NoMagnetic='無磁';
Blockly.LKL_NOVA_Hall_Magnetic='有磁';

Blockly.LKL_NOVA_ITR='紅外對射開關';
Blockly.LKL_NOVA_ITR_Noshelter='無遮擋';
Blockly.LKL_NOVA_ITR_Shelter='有遮擋';

Blockly.LKL_NOVA_POTENTIOMETER='電位器(0~100)';

Blockly.LKL_NOVA_RELAY='繼電器';
